...
describe("production release contract",()=>{
 ...
 it("uses a checkpointed migration flow",async()=>{
  ...
  for(const required of ["Recovery commit before Learning Site migration","checkout","Migration validation failed","Migrate project to Learning Site Engine"]){expect(services).toContain(required);}
 });
});
