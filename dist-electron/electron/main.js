...
function handle(channel, fn) { ipcMain.handle(channel, async (_event, ...args) => { try {
...
async function boot() { window = new BrowserWindow({ width: 1400, height: 900, minWidth: 1000, minHeight: 650, webPreferences: { contextIsolation: true, nodeIntegration: false, sandbox: true, preload: join(import.meta.dirname, "preload.cjs") } }); const url = process.env.VITE_DEV_SERVER_URL; if (url)
...
app.whenReady().then(async () => {
    ...
    handle("editor:deletePage", (p, file) => service.deletePage(root(p), String(file)));
    handle("editor:settings", p => service.settings(root(p)));
    handle("editor:saveSettings", (p, settings) => service.saveSettings(root(p), settings));
    ...
    handle("editor:selectTheme", (p, id) => service.selectTheme(root(p), String(id)));
    handle("editor:media", p => service.media(root(p)));
    handle("editor:importImage", p => service.importImage(root(p)));
    handle("editor:deleteImage", (p, file, force) => service.deleteImage(root(p), String(file), force === true));
    handle("editor:mediaReferences", (p, file) => service.mediaReferences(root(p), String(file)));
    ...
    handle("editor:createFolder", (p, name, parent) => service.createFolder(root(p), String(name), typeof parent === "string" ? parent : ""));
    handle("editor:renameFolder", (p, from, to) => service.renameFolder(root(p), String(from), String(to)));
    handle("editor:movePage", (p, source, target) => service.movePage(root(p), String(source), typeof target === "string" ? target : ""));
    ...
    handle("git:sync", async (p) => { await service.git(root(p), ["fetch", "origin"]); return service.git(root(p), ["pull", "--ff-only"]); });
    handle("git:commit", (p, message) => service.git(root(p), ["add", "-A"]).then(() => service.git(root(p), ["commit", "-m", String(message)])));
    handle("git:push", p => service.git(root(p), ["push", "-u", "origin", "HEAD"]));
    handle("git:history", p => service.git(root(p), ["log", "--oneline", "-30"]));
    handle("git:identity", (p, name, email) => Promise.all([service.git(root(p), ["config", "user.name", String(name)]), service.git(root(p), ["config", "user.email", String(email)])]));
    handle("app:version", () => app.getVersion());
    handle("app:update", async () => !app.isPackaged ? { state: "development" } : autoUpdater.checkForUpdates());
    handle("app:installUpdate", () => autoUpdater.quitAndInstall());
    handle("app:chooseDirectory", async () => { const result = await dialog.showOpenDialog({ properties: ["openDirectory", "createDirectory"] }); return result.canceled ? undefined : result.filePaths[0]; });
