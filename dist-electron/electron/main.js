...
app.whenReady().then(async () => {
    ...
    handle("editor:moveFolder", (p, source, target) => service.moveFolder(root(p), String(source), target ? String(target) : ""));
    ...
    handle("editor:reorderPage", (p, id, direction) => service.reorderPage(root(p), String(id), direction === "up" ? "up" : "down"));
    ...
    handle("editor:restorePage", (p, name, folder) => service.restorePage(root(p), String(name), typeof folder === "string" ? folder : ""));
    handle("editor:trashFolder", (p, folder) => service.trashFolder(root(p), String(folder)));
    handle("editor:trashedFolders", (p) => service.trashedFolders(root(p)));
    handle("editor:restoreFolder", (p, name, folder) => service.restoreFolder(root(p), String(name), typeof folder === "string" ? folder : ""));
    ...
    handle("git:commit", (p, message) => service
        .git(root(p), ["add", "-A"])
    ...
    handle("git:identity", (p, name, email) => Promise.all([
