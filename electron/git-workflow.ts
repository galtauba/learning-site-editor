...
const compare = (a: string, b: string) => {
  const av = tagVersion(a),
    bv = tagVersion(b);
  if (!av || !bv) return 0;
  return av[0] - bv[0] || av[1] - bv[1] || av[2] - bv[2];
};
export const stableTags = (raw: string) =>
  [
    ...new Set(
      raw
        .split(/\r?\n/)
...
const optional = async (run: GitRun, args: string[]) =>
  run(args, false).catch(() => undefined);
...
export async function officialVersions(
  run: GitRun,
...
export async function inspectProject(
  ...
  try {
    const origin = (
        await optional(run, ["remote", "get-url", "origin"])
      )?.trim(),
      upstream = (
        await optional(run, ["remote", "get-url", "upstream"])
      )?.trim(),
      branch = (
        await optional(run, ["symbolic-ref", "--quiet", "--short", "HEAD"])
      )?.trim();
    if (!branch)
      return {
        branch: "",
    ...
    const dirty = Boolean(
      (await optional(run, ["status", "--porcelain"]))?.trim(),
    );
    let ahead = 0,
      behind = 0;
    if (origin) {
    ...
    const state: ProjectSyncState = dirty
      ? "DIRTY"
      : ahead && behind
        ? "DIVERGED"
        : ahead
...
export async function syncOrigin(run: GitRun, push = true): Promise<string> {
  const origin = (await optional(run, ["remote", "get-url", "origin"]))?.trim();
  if (!origin) throw new Error("This project has no origin remote.");
  const branch = (
    await optional(run, ["symbolic-ref", "--quiet", "--short", "HEAD"])
  )?.trim();
  if (!branch)
    throw new Error(
      "This repository has no first commit yet. Set the Git name and email, then create the initial commit before synchronizing.",
    );
  const dirty = Boolean(
    (await optional(run, ["status", "--porcelain"]))?.trim(),
  );
  if (dirty) await checkpoint(run, "pre-recovery");
  await run(["fetch", "origin", "--prune"]);
  const counts =
      (
        await optional(run, [
  ...
  if (push && (ahead || behind)) await run(["push", "-u", "origin", branch]);
  return ahead && behind
...
export async function applyOfficialUpdate(
  run: GitRun,
  tag: string,
): Promise<void> {
  const status = await inspectProject(run);
  if (!status.upstream) throw new Error("Official upstream is not configured.");
  if (!tagVersion(tag))
  ...
  if (status.origin) await run(["push", "-u", "origin", status.branch]);
}
